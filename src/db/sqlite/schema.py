"""SQL schema for database tables and views."""

post_init = "insert into users (id, tokens_limit, accounts_limit, language, commands, registration_time) values (0, 0, 0, 'en', 0, CURRENT_TIMESTAMP);"
...
WITH RECURSIVE slaves (leaf_id, id, tokens_limit) AS (
    SELECT owner_id, id, tokens_limit FROM users
    UNION ALL
    SELECT s.leaf_id, u.id, u.tokens_limit
    FROM users u
    JOIN slaves s ON u.owner_id = s.id
)
SELECT * FROM slaves;
